...
class JobImportInvocationArchiveWrapper:
    """
        Class provides support for performing jobs that import a workflow invocation from
        an archive.
    """
...
class JobExportInvocationArchiveWrapper:
    """
    Class provides support for performing jobs that export a workflow instance to an
    archive.
    """
    ...
    def setup_job(self, jeia, include_hidden=False, include_deleted=False):
        """ Perform setup for job to export a workflow invocation into an archive. 
            Method generates attribute files for export, sets the corresponding attributes 
            in the jeia object, and returns the inputs and options that are used to run the tool
            defined in exp_workflow_instance_to_archive.xml; it does not include the output
            file because it must be set at runtime. """
        ...
        #
        # Create and return parameters for running tool.
        #
        options = "--galaxy-version '%s'" % VERSION_MAJOR
        if jeia.compressed:
            options += " -G"
